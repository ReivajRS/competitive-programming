...
signed main() {__
    ...
    FOR(i,0,n2){
        while (area(convex[i],convex[(i+1)%n2],convex[j]) < area(convex[i],convex[(i+1)%n2],convex[(j+1)%n2]))
            j = (j + 1) % n2;

        ld res = area(convex[i],convex[(i+1)%n2],convex[j]) / dist(convex[i],convex[(i+1)%n2]);
        ans = min(ans,res);
    }// no es
