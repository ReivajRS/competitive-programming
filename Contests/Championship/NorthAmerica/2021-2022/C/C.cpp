...
signed main() {
  ...
  FOR(i,0,n){
    ...
    if(it != ss.end()){
      brincos[i] = max(st.query(curr+1,(*it)),brincos[i]);
    }
    if(it != ss.begin()){
      it--;
      brincos[i] = max(st.query((*it) +1,curr),brincos[i]);
    }
    ss.insert(curr);
  }
  ...
  dp[0] = 0;
  FOR(i,0,n){
    dp[i+brincos[i]] = min(dp[i+brincos[i]],dp[i] + 3);
